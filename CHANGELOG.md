...
- Postgres connection pool: `pool_pre_ping` and `pool_recycle` on preview Neon — avoids HTTP 500 (`AdminShutdown`) after database scale-to-zero.

### Changed

- Accept `postgres://` in `DATABASE_URL` normalization (Neon/Railway-style URLs).
